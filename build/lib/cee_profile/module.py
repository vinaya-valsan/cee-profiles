...
class read_data:
    ...
    def get_full_radial_profile(
        self, fileid, radial_bins=None, weight_filed=None
    ) -> dict:
        ...
        if radial_bins is None:
            radius_bins = 10.0 ** np.arange(
                np.log10(min(Radius)), np.log10(max(Radius)), 0.05
            )
        else:
            RadialValues, Temperature_hist = CreateRadialProfile(
                Radius, Temperature, bins_x=radius_bins, weight_field=Mass
            )
            _, IE_hist = CreateRadialProfile(
                Radius, IE, bins_x=radius_bins, weight_field=Mass
            )
            _, Velocity_hist = CreateRadialProfile(
                Radius, Velocity, bins_x=radius_bins, weight_field=Mass
            )
            _, Density_hist = CreateRadialProfile(
                Radius, Density, bins_x=radius_bins, weight_field=Volume
            )
